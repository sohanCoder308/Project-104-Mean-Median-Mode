...
def calculateMean(weight, entries):
    mean = weight / entries
    print(f"Mean is = {mean:2f}")

def calculateMedian(entries, sorted_data):
    if entries % 2 == 0:
        median1 = float(sorted_data[entries//2])   
        median2 = float(sorted_data[entries//2 - 1]) 
        median = (median1 + median2) / 2
    else:
        median = float(sorted_data[entries//2])

    print(f"Median is = {median:2f}")    

def calculateMode(sorted_data):
...
calculateMean(total_weight, total_entries)
calculateMedian(total_entries, sorted_data)
calculateMode(sorted_data)